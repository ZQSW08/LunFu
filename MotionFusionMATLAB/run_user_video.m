...
%% 1. 输入、输出与采样率
u.videoPath='C:\0819\sun-3050-30mvpp.mp4';
u.outputRoot= 'D:\LunFu\MotionFusionMATLAB\outputs\1\1\1'; % 自动追加视频名，旧结果归档
u.captureFPS= 300;            % 实际采集帧率；[]才使用文件元数据
...
%% 3. 宏观运动参考
u.referenceModel='none';  % translation（一个参考） / similarity(至少两个参考) / 无参考 none
...
u.targetMode='direct';    % direct 一致目标配准 / profile 旧方法 / consensus 实验 / texture 双向
...
u.analysisBandHz=[2 60];         % []不带通；明确设置频带才生成额外诊断图
u.denoise=true;            % []频带时必须 false ；设true前先填写analysisBandHz
u.showFigures=true;          % 保存的FIG始终Visible=on
u.exportFigures=true;
u.exportTrackingVideo=false;     % 独立导出 tracking_overlay.avi，不计入算法耗时

result=run_real_video(u);
% 优先检查02_waveform中的总位移、参考位移、原始相对位移和有效性。
% 有效率不等于精度；三角光/激光数据不进入本脚本的跟踪、参数或频谱处理，
% 只能在本次视频测量完成后由 compare_laser_v2 独立评价。
