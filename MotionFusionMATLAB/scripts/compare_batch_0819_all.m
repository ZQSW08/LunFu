...
%% 1. 输入、输出与采样率
videoOutputRoot=fullfile(projectRoot,'outputs','batch_0819_all');
sensorRoot='E:\sanjiao\0819';
sensorFPS=100;                         % 三角光采样率：CSV无时间列，按实验约定100 Hz
videoAxis='x';                         % 与批处理脚本的测量方向保持一致
comparisonFolderName='comparison_0819';
...
if ~isempty(rows)
    ...
    writetable(summary,fullfile(videoOutputRoot,'comparison_0819_metrics.csv'));
    disp(summary);
else
    warning('没有找到可以同时匹配的07-26视频结果和三角光文件。');
end
fprintf('\n0819相对波形对比完成。三角光采样率按100 Hz记录；未进行物理标定。\n');
